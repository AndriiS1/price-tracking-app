    ...
    ISender mediator) : ControllerBase
{
    ...
    [AllowAnonymous]
    [HttpGet]
    [Route("tracked-products/{id}")]
    public async Task<IActionResult> TrackedProductController(string id)
    {
        return await mediator.Send(new GetTrackedProductQuery { Id = id });
    }
}